...
if (isset($_POST['winner'])) {
  ...
  $stmt = $conn->prepare("DELETE FROM winner_scheme_3 WHERE  id = ?");
...
<div class="row">
    ...
    <table class="table">
            ...
            <tbody>
              <?php
                include 'partials/dbconnect.php';
                $sql = "SELECT * FROM winner_scheme_3";
                ...
                if ($result->num_rows > 0) {
                  while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td><form method='POST'> <input type='hidden' name='id' value='" . $row['id'] . "'><input type='hidden' name='winning_number' value='" . $row["five_digit_number"] . "'><input type='hidden' name='username' value='" . $row["username"] . "'><button class='btn btn-primary' name='winner'>Send to previous winner</button></form></td>"; // add button to each row
                    echo "</tr>";
                  }
                } else {
                  echo "0 results";
