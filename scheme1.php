...
if (isset($_POST['submit'])) {
  ...
  // Retrieve form data
  $three_digit_number = mysqli_real_escape_string($conn, $_POST['three_digit_number']);
  ...
  $sql = "SELECT COUNT(*) as count FROM form_1_ban_number WHERE ban_number = ?";
  $stmt = mysqli_prepare($conn, $sql);
  mysqli_stmt_bind_param($stmt, "s", $three_digit_number);
  mysqli_stmt_execute($stmt);
  ...
  if ($ban_row['count'] > 0) {
    ...
    $sql = "INSERT INTO form_1 (user_id, three_digit_number, amount, result, dt, user_name) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "isssss", $user_id, $three_digit_number, $amount, $result_input, $dt, $user_name);
...
<html lang="en">
  ...
  <script>
		function updateResult() {
			...
			document.querySelector('input[name="result"]').value = amount * 1000;
		}
	</script>
  <body>
      ...
      <div class="row justify-content-center">
		...
		<form method="post">
			<label for="three_digit_number">Three Digit Token:</label>
			<input type="text" name="three_digit_number" placeholder="Enter your number" pattern="\d{3}" maxlength="3" required>
			<br><br>
			<label for="amount">Coin Amount</label>
			<input type="number" placeholder="Enter your coin amount" name="amount" oninput="updateResult()" required>
			<br><br>
