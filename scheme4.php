...
if (isset($_POST['submit'])) {
  ...
  // Retrieve form data
  $six_digit_number = mysqli_real_escape_string($conn, $_POST['six_digit_number']);
  ...
  $sql = "SELECT COUNT(*) as count FROM form_4_ban_number WHERE number = ?";
  ...
  if ($ban_row['count'] > 0) {
    ...
    $sql = "INSERT INTO form_4 (user_id, six_digit_number, amount, result, dt, user_name) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
...
<html lang="en">
  ...
  <script>
		function updateResult() {
			...
			document.querySelector('input[name="result"]').value = amount * 100000;
		}
	</script>
  <body>
      ...
      <div class="row justify-content-center">
		...
		<form method="post">
			<label for="six_digit_number">Six Digit Token:</label>
			<input type="text" name="six_digit_number" placeholder="Enter your number" pattern="\d{6}" maxlength="6" required>
			<br><br>
