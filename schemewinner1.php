...
if (isset($_POST['winner'])) {
  ...
  $stmt = $conn->prepare("DELETE FROM winner WHERE three_digit_number = ?");
              ...
              <?php
                ...
                if ($result->num_rows > 0) {
                  while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td><form method='POST'><input type='hidden' name='winning_number' value='" . $row["three_digit_number"] . "'><input type='hidden' name='username' value='" . $row["username"] . "'><button class='btn btn-primary' name='winner'>Send to previous winner</button></form></td>"; // add button to each row
                    echo "</tr>";
                  }
                } else {
                  echo "0 results";
