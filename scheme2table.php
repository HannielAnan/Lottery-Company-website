...
if (isset($_POST['number'])) {
  ...
  $stmt = $mysqli->prepare("INSERT INTO form_2_ban_number (number, ban_until) VALUES (?, ?)");
...
if (isset($_POST['winner'])) {
 ...
 mysqli_query($conn, "SET FOREIGN_KEY_CHECKS=0");
  ...
  $stmt = $conn->prepare("INSERT INTO winner_scheme_2 (four_digit_number, username) VALUES (?, ?)");
...
<button type="button" class="btn btn-danger" onclick="window.location.href='deletenumber2.php'" data-bs-toggle="modal" style="margin-left: 30px;">
 Delete Ban Numbers
</button>
<button type="button" class="btn btn-danger" onclick="window.location.href='schemewinner2.php'" data-bs-toggle="modal" style="margin-left: 30px;">
 Scheme 2 winner
</button>
...
if (isset($_POST['date'])) {
  ...
  $query = "SELECT * FROM form_2 WHERE DATE(dt) = '$date'";
  $result = mysqli_query($conn, $query);
} else {
  // Fetch all data from the "form_1" table
  $query = "SELECT * FROM form_2";
  $result = mysqli_query($conn, $query);
}
    ...
    if (mysqli_num_rows($result) > 0) {
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<td><form method='POST'><input type='hidden' name='winning_number' value='" . $row["four_digit_number"] . "'><input type='hidden' name='username' value='" . $row["user_name"] . "'><button class='btn btn-primary' name='winner'>Winner</button></form></td>"; // add button to each row
        echo "</tr>";
