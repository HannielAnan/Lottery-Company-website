...
$mysqli->query("SET FOREIGN_KEY_CHECKS=0");
  ...
  // Prepare the SQL statement to insert the data into the winner table
$stmt = $conn->prepare("INSERT INTO iphone_winner (seven_digitiphone_number, username) VALUES (?, ?)");
...
<a href="iphonenumchange.php"><button type="button" class="btn btn-danger" data-bs-toggle="modal" style="margin-left: 10px;">
  Change Fix Number
</button></a>
<button type="button" class="btn btn-danger"  data-bs-target="#winnerModal" data-bs-toggle="modal" style="margin-left: 30px;">
Announce winner
</button>
<button type="button" class="btn btn-danger" onclick="window.location.href='iphonewinner.php'" data-bs-toggle="modal" style="margin-left: 30px;">
 Iphone winner
</button>
    <?php
    // Connect to the database
...
if (isset($_POST['date'])) {
  ...
  $query = "SELECT * FROM iphone WHERE DATE(dt) = '$date'";
  ...
  $result = mysqli_query($conn, $query);
    ...
    if (mysqli_num_rows($result) > 0) {
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<td>" . $row["seven_digitiphone_number"] . "</td>";
        echo "<td>" . $row["amount"] . "</td>";
        echo "<td>" . $row["dt"] . "</td>";
        echo "<td>" . $row["user_name"] . "</td>";
        echo "<td><form method='POST'><input type='hidden' name='winning_number' value='" . $row["seven_digitiphone_number"] . "'><input type='hidden' name='username' value='" . $row["user_name"] . "'><button class='btn btn-primary' name='winner'>Winner</button></form></td>"; // add button to each row
        echo "</tr>";
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="banModalLabel" style="color:black;">Ban User</h5>
      ...
      <div class="modal-body">
        <form method="post">
          ...
          <div class="mb-3">
            <label for="ban_duration" class="form-label" style="color:black;">Ban duration (in hours):</label>
