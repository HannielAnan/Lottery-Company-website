...
if (isset($_POST['number'])) {
  ...
  $stmt = $mysqli->prepare("INSERT INTO form_car_ban_number (number, ban_until) VALUES (?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['amount'])) {
  ...
  $stmt = $mysqli->prepare("INSERT INTO fixed_amounts (amount) VALUES (?)");
...
$mysqli->query("SET FOREIGN_KEY_CHECKS=0");
  ...
  $stmt = $mysqli->prepare("INSERT INTO car_winner (seven_digit_number, username) VALUES (?, ?)");
...
<button type="button" class="btn btn-danger" onclick="window.location.href='cardelete.php'" data-bs-toggle="modal" style="margin-left: 30px;">
 Delete Ban Numbers
</button>
<a href="carnumchange.php"><button type="button" class="btn btn-danger" data-bs-toggle="modal" style="margin-left: 10px;">
  Change Fix Number
</button></a>
<button type="button" class="btn btn-danger"  data-bs-target="#winnerModal" data-bs-toggle="modal" style="margin-left: 30px;">
Announce winner
</button>
<button type="button" class="btn btn-danger" onclick="window.location.href='carwinner.php'" data-bs-toggle="modal" style="margin-left: 30px;">
 Car winner
</button>
    <?php
    // Connect to the database
...
if (isset($_POST['date'])) {
  ...
  $query = "SELECT * FROM car WHERE DATE(dt) = '$date'";
  ...
  $result = mysqli_query($conn, $query);
    ...
    if (mysqli_num_rows($result) > 0) {
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<td>" . $row["seven_digit_number"] . "</td>";
        echo "<td>" . $row["amount"] . "</td>";
        echo "<td>" . $row["dt"] . "</td>";
        echo "<td>" . $row["user_name"] . "</td>";
        echo "<td><form method='POST'><input type='hidden' name='winning_number' value='" . $row["seven_digit_number"] . "'><input type='hidden' name='username' value='" . $row["user_name"] . "'><button class='btn btn-primary' name='winner'>Winner</button></form></td>"; // add button to each row
        echo "</tr>";
