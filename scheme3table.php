...
if (isset($_POST['number'])) {
  ...
  $stmt = $mysqli->prepare("INSERT INTO form_3_ban_number (number, ban_until) VALUES (?, ?)");
...
mysqli_query($conn, "SET FOREIGN_KEY_CHECKS=0");
  ...
  $stmt = $conn->prepare("INSERT INTO winner_scheme_3 (five_digit_number, username) VALUES (?, ?)");
...
<html lang="en">
  ...
  <tbody>
  <!-- <form action="scheme1table.php" method="get">
...
<button type="button" class="btn btn-danger" onclick="window.location.href='deletenumber3.php'" data-bs-toggle="modal" style="margin-left: 30px;">
 Delete Ban Numbers
</button>
<button type="button" class="btn btn-danger" onclick="window.location.href='schemewinner3.php'" data-bs-toggle="modal" style="margin-left: 30px;">
 Scheme 3 winner
</button>
...
if (isset($_POST['date'])) {
  ...
  $query = "SELECT * FROM form_3 WHERE DATE(dt) = '$date'";
  $result = mysqli_query($conn, $query);
} else {
  // Fetch all data from the "form_1" table
  $query = "SELECT * FROM form_3";
  $result = mysqli_query($conn, $query);
}
    ...
    if (mysqli_num_rows($result) > 0) {
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<td>" . $row["five_digit_number"] . "</td>";
        echo "<td>" . $row["amount"] . "</td>";
        echo "<td>" . $row["result"] . "</td>";
        echo "<td>" . $row["dt"] . "</td>";
        echo "<td>" . $row["user_name"] . "</td>";
        echo "<td><form method='POST'><input type='hidden' name='winning_number' value='" . $row["five_digit_number"] . "'><input type='hidden' name='username' value='" . $row["user_name"] . "'><button class='btn btn-primary' name='winner'>Winner</button></form></td>"; // add button to each row
        echo "</tr>";
